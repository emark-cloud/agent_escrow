...
const NETWORK = process.env.NETWORK || "bradbury"; // "bradbury" or "studionet"
...
async function apiCall(method, endpoint, walletId, body, useWait = true) {
  ...
  const headers = { "x-api-key": API_KEY, "Content-Type": "application/json", "x-network": NETWORK };
...
async function main() {
  ...
  const health = await apiCall("GET", "/api/health");
  logSystem(`Network: ${health.network || NETWORK}`);
