import { AGREEMENT_STATUS, MILESTONE_STATUS, STATUS_COLORS, AGREEMENT_STATUS_COLORS } from "@/lib/config";

export function AgreementStatusBadge({ status }: { status: number }) {
  return (
    <span
      className={`inline-flex items-center px-2.5 py-1 rounded-full text-xs font-medium text-white ${AGREEMENT_STATUS_COLORS[status] || "bg-gray-500"}`}
    >
